...
export class FastStrategy extends BaseStrategy {
    async execute(query, intent, sessionId, user, res, history) {
        ...
        try {
            ...
            // Memory Lookup
            const memoryContext = Promise.resolve(MemoryService.getMemoriesForContext(sessionId, query)); // Start parallel
            ...
            // 3. Lightweight Rerank (Skip LLM Reranker if possible)
            // Trust Hybrid Score directly.
            ...
            // Update Thoughts: Show Filtered Count
            ...
            // Deduplication (ID-based for citations)
            const uniqueDocs = [];
            ...
            // 5. Build Context (Skipped mapping since LlamaIndex provides contextText directly)

            // Prepare Images (Threshold: 0.3 for Fast Mode junk filter)
            ...
            // Strict RAG: ALWAYS use RAG System Prompt.
            // If no docs, contextText will be empty string, triggering "Not Found" response.

            let systemPrompt = '';
            let effortInstruction = '';

            // Reasoning Effort Display & Instruction
            ...
            const mem = await memoryContext;
            // Pass Role Hint from Intent
            // Note: contextText might be empty string if no docs found.
            systemPrompt = SystemPrompts.RAG_SYSTEM_PROMPT_TEMPLATE(
